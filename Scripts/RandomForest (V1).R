...
ncores  <- as.numeric(args[5]) # ncores <-  15
...
###################################################################################################################################
##############################              Select the best parameters for the best model               ###########################
###################################################################################################################################
...
###################################################################################################################################
##############################                 Obtain the most popular feature selected                 ###########################
###################################################################################################################################
...
DF_TRAIN <- MEspDF[,match(c( "Variable" , names(Best_features) ) , colnames(MEspDF) )]	
BEST_MODEL_FULL <- train(Variable ~ ., data = DF_TRAIN, method = "rf", trControl = ctrl, tuneGrid = df_best_hyperparameter)	
saveRDS(file = "BEST_MODEL_FULL.rds",BEST_MODEL_FULL)

if (is_imbalanced == T) {
	Train_selected <- smart_smote(data = DF_TRAIN) 
	BEST_MODEL_FULL_BALANCED <- train(Variable ~ ., data = Train_selected, method = "rf", trControl = ctrl, tuneGrid = df_best_hyperparameter)
	saveRDS(file = "BEST_MODEL_FULL_BALANCED.rds",BEST_MODEL_FULL_BALANCED)
}
